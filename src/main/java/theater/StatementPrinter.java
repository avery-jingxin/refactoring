...
public class StatementPrinter {
    ...
    public String statement() {
        ...
        for (Performance p : invoice.getPerformances()) {
            result.append(String.format("  %s: %s (%s seats)%n", plays.get(p.getPlayID()).getName(), usd(getAmount(p)),
                    p.getAudience()));
        }
    ...
    private static int getAmount(Performance performance) {
        int rslt = 0;
        switch (getPlay(plays.get(performance.getPlayID())).getType()) {
            case "tragedy":
            ...
            default:
                throw new RuntimeException(String.format("unknown type: %s",
                        plays.get(performance.getPlayID()).getType()));
        }
        return rslt;
    }

    private static Play getPlay(Play play) {
        return play;
    }
}
